colorAdminApp.controller('adminAttendanceInOutReportController', function ($scope, $rootScope, $http, $state, DTOptionsBuilder, DTColumnBuilder, $filter, $compile) {
 ...
 var startDate = new Date();
    ...
    function getAttendanceInOutReport(sSource, aoData, fnCallback, oSettings) {
        ...
        $http.get($scope.webApiUrl + 'admin/GetAttendanceInOutReport?apiKey=' + $scope.school.appKey + '&adminId=' + $scope.user.entityId +'&roleId=' + $scope.search.role.roleId + '&groupId=' + $scope.search.group.groupId + '&classId=' + $scope.search.class.groupId + "&academicYearId=" + $scope.academicYear.academicYearId +
            '&startDate=' + $scope.search.startDate + '&endDate=' + $scope.search.endDate).success(function (result) {
                if (result.status == true) {
                    $scope.teacherInOutReport = result.data;
                    var records = {
                        'draw': draw,
   ...
   $http.post($scope.webApiUrl + 'admin/GetRoleAttendance', JSON.stringify({
        ...
        $http.post($scope.webApiUrl + 'admnin/GetClassList', JSON.stringify({
            apiKey: $scope.school.appKey, academicYearId: $scope.academicYear.academicYearId, screenId: $scope.screenId, adminId: $scope.user.entityId
