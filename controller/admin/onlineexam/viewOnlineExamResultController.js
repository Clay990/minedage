colorAdminApp.controller('adminViewOnlineExamResultController', function ($scope, $rootScope, $http, $state, DTOptionsBuilder, DTColumnBuilder, $filter, $compile) {
    $scope.user = JSON.parse(window.localStorage.getItem("user"));
    $scope.academicYear = JSON.parse(window.localStorage.getItem("academicYear"));
    $scope.school = JSON.parse(window.localStorage.getItem("school"));
     var startDate = new Date();

    $scope.search = { startDate:$scope.formatDate(new Date()), endDate: $scope.formatDate(new Date()), group: { groupId: 0 },section: { sectionId: 0 }, subject: { subjectId: 0 } };
    $scope.screenId = 1003;
    $scope.draw;
    $scope.column;
    $scope.order;
 ...
 $scope.getSections = function () {
  ...
  DTColumnBuilder.newColumn("resultDescription", "Teacher Remark").withOption('name', 'Remark').withClass("none").renderWith(function (data, type, full) {
    ...
    $scope.viewOnlineResultDocument = function (onlineExamId,onlineExamTypeId,studentId,onlineExamResultId) {
                        ...
                        $state.go('app.admin.onlineExam.viewOnlineExamWorkFlowDocument', { onlineExamId: onlineExamId,onlineExamTypeId:onlineExamTypeId,studentId:studentId,onlineExamResultId:onlineExamResultId});
                    }
