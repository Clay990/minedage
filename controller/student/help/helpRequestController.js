colorAdminApp.controller('studentHelpRequestController', function ($scope, $rootScope, $http, $state, DTOptionsBuilder, DTColumnBuilder, $filter, $compile) {
    $scope.student = JSON.parse(window.localStorage.getItem("student"));
   ...
   var startDate = new Date();
    ...
    $scope.getHelps = function () {
        $http.get($scope.webApiUrl + 'student/GetHelpRequest?apiKey=' + $scope.school.appKey + '&studentId=' + $scope.user.entityId + "&academicYearId=" + $scope.academicYear.academicYearId +
    ...
    $scope.getHelpDetail = function (help) {
        ...
        $http.get($scope.webApiUrl + 'student/GetHelpWorkFlow?apiKey=' + $scope.school.appKey + "&helpRequestId="+ help.helpRequestId +"&academicYearId=" + $scope.academicYear.academicYearId
          ).success(function (result) {
                if (result.status == true) {
    ...
    $scope.submitHelp = function (form) {
        ...
        if ($scope.helpForm.$valid) {
            ...
            $http.post($scope.webApiUrl + 'shared/CreateHelp?apiKey=' + $scope.school.appKey + '&academicYearId=' + $scope.academicYear.academicYearId , helpData).success(function (result) {
                if (result == null) {
                    $scope.error("An unhandled error has occured. Please try after sometime, and contact your administrator if the problem persists.", true);
                }
                else {
